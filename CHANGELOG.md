...
## [0.2.0](https://github.com/OxideAV/oxideav-mjpeg/compare/v0.1.8...v0.2.0) - 2026-09-11
...
- README frame-level snippet compiles (fn instead of a borrowing closure)
- general T.81 writer section (API tiff calls, consumer re-point note), decoder coverage for any sampling layout / abbreviated streams / non-interleaved extents, twelve fuzz targets
- rustfmt the t81_roundtrip target
- t81_roundtrip — writer→reader pair over process × precision × layout × tables × restart × abbreviated
- gate registry_options behind the registry feature (standalone build)
- typed encoder options (CodecOptions ↔ JpegEncodeOptions) + §B.5 abbreviated decode via extradata tables
- general T.81 writer — sequential 8/12-bit, progressive, lossless; any sampling; optimal K.2 tables; abbreviated streams; typed options
- non-interleaved scans cover ceil(xi/8) × ceil(yi/8) blocks, not the MCU-padded grid (T.81 §A.2.2 / §A.2.4)
- decode every T.81 §A.1.1 sampling-factor combination (4×2 luma, mixed chroma, …)
- apply the T.81 §H.1.2.1 restart rule, reserve the all-ones code (§C.2), fix RGB/CMYK signalling (T.871)
